...
export const meta: MetaFunction = () => {
  return [
    { title: "Bright Red Comic Archive | Willy + Froggy" },
    {
      name: "description",
      content:
        "Bright Red is a weekly humour strip that is associated with a forthcoming adventure comic that will be called The Bright Red Time Potion. Browse the archive to read all past comics.",
    },
...
export default function Comics() {
  ...
  const [bannerImageIsLoading, setBannerImageIsLoading] = useState(true);

  useEffect(() => {
    // check if the image is loaded, if so, set comicIsLoading to false
    const bannerImage = document.getElementById("BR-archive-banner");

    if (bannerImage) {
      setBannerImageIsLoading(false);
    }
  }, []);
  ...
  return (
    <main className="BR-archive">
      <Image
        id="BR-archive-banner"
        className={bannerImageIsLoading ? "hide-image" : ""}
